# Digital Ambassador Project

Name: Lavanya  
Registration Number: 25BAI11378 
Course/Branch: Computer Science Specialisation in Artificial Intelligence and Machine Learning 
Year: 1st Year

This repository contains my **Digital Ambassador Project**, covering 5 modules aimed at enhancing digital literacy, professional online presence, coding and collaboration skills, email etiquette, and cybercrime awareness.

---

## Module 1 – Digital Literacy Infographic
**Folder:** task-1-presentation 
- `infographic.png` → One-page visual infographic on digital literacy  
- `report.txt` → Reflection notes on visual infographic  

**Canva Link:** 
- [Canva Infographic Link] https://www.canva.com/design/DAHEzOrCoqw/3mh7FKb7sPJvgQGpkJ6k8g/edit?utm_content=DAHEzOrCoqw&utm_campaign=designshare&utm_medium=link2&utm_source=sharebutton

---

## Module 2 – Student Digital Portfolio
**Folder:** `task-2-portfolio  
- `github-profile.png` → Screenshot of GitHub profile with README  
- `linkedin-profile.png` → Screenshot of LinkedIn education section  
- `report.txt` → Reflection notes on setting up profiles  

---

## Module 3 – Coding & Collaboration Platforms
**Folder:** `task-3-platforms  
- `hackerrank.png` → Screenshot of completed HackerRank challenge  
- `form.png` → Screenshot of Digital Literacy Awareness Quiz form  
- `sheet.png` → Screenshot of linked Google Sheet responses  
- `report.txt` → Reflection notes on using coding and collaboration tools  

**Google Form Link:**  
https://docs.google.com/forms/d/e/1FAIpQLScMngKei_TLspwx6WhEW6CkWO6qxve5oiu2k19Dv2r7WYYjbg/viewform?usp=publish-editor

---

## Module 4 – Professional Email & Social Media Etiquette
**Folder:** `task-4-email-etiquette
- `email1.docx` → Draft email requesting assignment extension  
- `email2.docx` → Draft email expressing interest in internship  
- `social-media-checklist.txt` → Do’s and Don’ts for social media  
- `project-report.docx` → Reflection notes on email etiquette and social media  

---

## Module 5 – Cybercrime Awareness
**Folder:** `task-5-cybercrime  
- `casestudy.md` → Case study on UPI/Online Payment Fraud  
- `prevention-checklist.md` → "Stay Safe Online" checklist with 8 tips  
- `project-report.txt` → Reflection notes on cybercrime awareness  

**Reporting Channels:**  
- National Cyber Crime Portal: [cybercrime.gov.in](https://cybercrime.gov.in)  
- Helpline: 1930  

---
  
- All screenshots and reports are stored in their respective folders.  
- This README serves as a guide to navigate through the project repository.
